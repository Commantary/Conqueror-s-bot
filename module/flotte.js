...
request.get(bdd_id, function (err, res, body1) {
	...
	if (data[message.author.id] !== undefined){
		...
		request.get(bdd, function (err, res, body) {
			...
			if (numbers !== 0 && Number(json[usernameOfPlayer].flottes)-Number(numbers) > 0){
				switch (numbers) {
				  case "0":
				...
				case "1":
				  ...
				  json[usernameOfPlayer].flottes -= numbers
				  var armyBefore = Number(json[usernameOfPlayer].flottes) + Number(numbers)
				  console.log(1)
				  message.channel.send({embed: {
				    color: 12434877,
				    description: "```Markdown\n# " + result + "\n```\n```Markdown\n" + result + "\n```\n```Markdown\n" + json[usernameOfPlayer].name + " est passer de " + armyBefore + " flottes à " + json[usernameOfPlayer].flottes + " et a fait un score de " + result + " avec ces flottes\n```"
				  }})
				  // On put tout sa!
				...
				message.channel.send({embed: {
					color: 16711744,
					description: "Il y a eu une erreur, oubliez pas que vous pouvez utiliser entre 1 et 10 flottes et que vous avez besoin d'avoir ces armées là bien-évidemment"
				}})
			} // FIN DU VERIF 0
		})
	} else {
